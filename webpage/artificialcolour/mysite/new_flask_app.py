...
def rand_colour():
    return f'#{random.randint(2**2, 2**6):02X}{random.randint(2**2, 2**6):02X}{random.randint(2**2, 2**6):02X}'
...
def colour_generator():
    ...
    if request.method == 'POST':
        ...
        n = 5 - len(hex_inputs)
        if len(hex_inputs) == 0:
            hex_inputs += [rand_colour()]
        # random.shuffle(hex_inputs)
        gen_out = generate_palette(hex_inputs)[:n]
        random.shuffle(gen_out)
...
def load_info():
    back_url = url_for('colour_generator')
    if 'col1' not in session:
        ...
        return f'hex {session.get("col1")} {session.get("col2")} {session.get("col3")} {session.get("col4")} {session.get("col5")} <a href="/">back</a>'
